...
function ShowResultsSearchForParade(str) {
  if (str.length==0) {
...
function ResultHasBeenClickedParade(parade_id, parade_date, parade_name) {
  document.getElementById("livesearch_parade_id").innerHTML="";
  document.getElementById("livesearch_parade_id").style.border="0px";
...
function REGEXCheckParade(str, input_to_check){
...
function REGEXCheckEvent(str, input_to_check){
  ...
  if (input_to_check === "event_type") {
    ...
    if (!str.match(/^\d{2}:\d{2}$/)) {
      event_feedback["event_start"] = "<a>Invalid time format (HH:MM)<a><br>";
    ...
    if (!str.match(/^\d{2}:\d{2}$/)) {
      event_feedback["event_end"] = "<a>Invalid time format (HH:MM)<a><br>";
...
function REGEXCheckAddUser(str, input_to_check){
  ...
  if (input_to_check === "email") {
    if (str.length > 255){
      ...
      add_user_array["email"] = 0;
    } else if (!str.match("^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}$")) {
