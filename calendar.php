    ...
    if(count($events) == 0){//the user has no events on this parade night
      ...
      while($event_count < count($events)){
        ...
        if($user_data["admin"] == 1){//admin so add buttons to activate the event specific element of the admin panle
          ...
          $event_html = $event_html . "<div class=\"" . $style_class . "\"><a>" . $events[$event_count]["event_start"] .  " till " . $events[$event_count]["event_end"] . "</a><br><a>" . $events[$event_count]["event_name"] . "</a><br><a>" . $owner["rank"] . " " . $owner["first_name"] . " " . $owner["last_name"] . "</a><br><button onclick=\"populateAdminEditEventForm('" . $events[$event_count]["parade_id"] . "', '" . $events[$event_count]["event_id"] . "', '" . $events[$event_count]["event_type"] . "', '" . $events[$event_count]["event_name"] . "', '" .  $events[$event_count]["event_start"] . "', '" . $events[$event_count]["event_end"] . "', '" . $events[$event_count]["owner"] . "', '" .  $events[$event_count]["final_aproval"] . "')\">click for admin panle edit</button></div>";
